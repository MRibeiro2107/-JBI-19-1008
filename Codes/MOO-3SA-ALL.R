PD_ENSEMBLE<-function(data,pp,cp,mp,models,seed1,seed2)
{
  ...
  PTRF   <-matrix(nrow=128+1,ncol=2*length(models)+1);
  ...
  PTRmo  <-matrix(nrow=128+1,ncol=length(models));
  ...
  for(i in 1:dim(data_dec)[2])
  {
    ...
    fitControl<- trainControl(method         = "timeslice",
                              ...
                              horizon        = 3,              #maximum forecasting horizon in the paper 
                              fixedWindow    = TRUE,
                              allowParallel  = TRUE,
    ...
    for(m in 1:length(models))
    {
      ...
      #------------------Recursive forecasting for train set-----------
      for(N in 1:3)
      {
        for(v in 1:43)
        {
        ...
        if(p !=129)
        {
          ...
          {
            X_trainm[p,Lag1]<-PTRmo[p-1,m]
            PTRmo[p,m]    <-predict(Models[[k]],X_trainm[p,])
          }
          else 
          {
            X_trainm[p,Lag1]<-PTRmo[p-1,m]
            X_trainm[p,Lag2]<-X_trainm[p-1,Lag1]
      ...
      for(N in 1:3)  #número de colunas do obs
      {
        for(v in 1:4)#número de linhas do obs
        {
          ...
          {
            X_testm[p,Lag1]<-PTEmo[p-1,m]
            PTEmo[p,m]    <-predict(Models[[k]],X_testm[p,])
          }
          else 
          {
            X_testm[p,Lag1]<-PTEmo[p-1,m]
            X_testm[p,Lag2]<-X_testm[p-1,Lag1]
    ...
    for(N in 1:3)
    {
      for(v in 1:43)
      {
        ...
        if(p != 129)
        {
          ...
          {
            X_trainm[p,Lag1]<-PTrOSA[p-1,c+m]
            PTrOSA[p,c+m]    <-predict(Models[[k]],X_trainm[p,])
          }
          else 
          {
  ...
  print(PFS)
  #--------------------------Save the results------------------------------------#
  Results3SA<-list(PTrOSA,PTeOSA,PTRF,PTEF,ETRF,ETEF,MF,Params,Models,WF,ParetoF,data_dec,PFS,P)
  form2   <-paste("Results_Ensemble_",Sys.Date(),"_State_",data[1,2],"3SA.RData", sep='')
  
  setwd(ResultsDir)
  save(Results3SA,file=form2)
