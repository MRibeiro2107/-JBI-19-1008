PD_ENSEMBLE<-function(data,pp,cp,mp,models,seed1,seed2)
{
  ...
  #Aditional objects
  obsTR<-matrix(seq(1,129,1),nrow=43,ncol=3,byrow=TRUE)
  obsTE<-matrix(seq(1,12,1),nrow=4,ncol=3,byrow=TRUE)
  ...
  PTrOSA <-matrix(nrow=128+1,ncol=dim(combs)[1]+length(models)+1); 
  ...
  for(i in 1:dim(data_dec)[2])
  {
    ...
    fitControl<- trainControl(method         = "timeslice",
                              initialWindow  = floor(cut*0.7), #70% 
    ...
    for(m in 1:length(models))
    {
      ...
      h=2
      #Recursive forecasting in training
      for(p in 1:128)
      {
        if(p%%h !=0)
        {
          PTRmo[p,m]<-predict(Models[[k]],X_trainm[p,])
          X_trainm[p+1,Lag1]<-PTRmo[p,m]
        }
        else
        {
          PTRmo[p,m]   <-predict(Models[[k]],X_trainm[p,])
      ...
      #Recursive forecasting in test set
      
      for(p in 1:12)
      {
        if(p%%h !=0)
        {
          PTEmo[p,m]<-predict(Models[[k]],X_testm[p,])
          X_testm[p+1,Lag1]<-PTEmo[p,m]
        }
        else
        {
          PTEmo[p,m]   <-predict(Models[[k]],X_testm[p,])
  ...
  for(c in 1:dim(combs)[1])
  {
    ...
    PTrOSA[1:128,c]<-floor((PTR[[1]][,combs[c,1]]*thetas[c,1]+
                         PTR[[2]][,combs[c,2]]*thetas[c,2]+
                         PTR[[3]][,combs[c,3]]*thetas[c,3]+
                         PTR[[4]][,combs[c,4]]*thetas[c,4]+
                         PTR[[5]][,combs[c,5]]*thetas[c,5]))
  ...
  for(m in 1:length(models))
  {
    ...
    h=2
    #Recursive forecasting for training
    for(p in 1:128)
    {
      if(p%%h !=0)
      {
        PTrOSA[p,c+m]<-predict(Models[[k]],X_trainm[p,])
    ...
    for(p in 1:12)
    {
      if(p%%h !=0)
      {
        PTeOSA[p,c+m]<-predict(Models[[k]],X_testm[p,])
    ...
    ETrOSA[,c+m]<-data[5:132,1]-floor(PTrOSA[1:128,c+m])
    ETeOSA[,c+m]<-data[133:144,1]-floor(PTeOSA[1:12,c+m])
    
    #-----------------------------Performance----------------------------------------#
    P[c+m,1:4]<-PM(data[5:132,1]  ,floor(PTrOSA[1:128,c+m]))
    P[c+m,6:9]<-PM(data[133:144,1],floor(PTeOSA[,c+m]))
  ...
  print(PFS)
  #--------------------------Save the results------------------------------------#
  Results2SA<-list(PTrOSA,PTeOSA,PTRF,PTEF,ETRF,ETEF,MF,Params,Models,WF,ParetoF,data_dec,PFS,P)
  form2   <-paste("Results_Ensemble_",Sys.Date(),"_State_",data[1,2],"2SA.RData", sep='')
  
  setwd(ResultsDir)
  save(Results2SA,file=form2)
